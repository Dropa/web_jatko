...
myApp.controller("appController", function($scope){
    ...
    $scope.addNewSearch = function ($newSearch) {
      if ($newSearch != null) {
          $scope.searches.push({'search': $newSearch});
          $scope.currentSearch = $newSearch;
          $scope.newSearch = "";
      }
    };

    $scope.setSearch = function ($searchIndex) {
      $scope.currentSearch = $scope.searches[$searchIndex].search;
    };
    $scope.headers = [
        {'ID*1': 'ID'},
        {'name': 'Name'},
    ...
    $scope.sorts = 'ID';
    $scope.reverse = false;
    $scope.sortBy = function ($arrayKey) {
        if($scope.sorts = $arrayKey) {
            $scope.reverse = !$scope.reverse;
        }
        $scope.sorts = $arrayKey;
    };
    $scope.countries = [
