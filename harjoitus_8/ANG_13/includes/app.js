...
myApp.controller("employeesController", function($scope, $http) {
    ...
    $scope.updateEmployee = function(updateemployee) {
        var updateEmployee = angular.copy(updateemployee);
        var year = updateEmployee.bdate.getFullYear();
        var month = ("0" + (updateEmployee.bdate.getMonth() + 1)).slice(-2);
        var day = ("0" + updateEmployee.bdate.getDate()).slice(-2);
        updateEmployee.bdate = year + "-" + month + "-" + day;

        $http.post("http://home.tamk.fi/~kujesa/webjatko/rest/index.php/updateEmployee", updateEmployee, {
            headers : {
                'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'
            }
        }).then(function(r) {
    ...
    $scope.getUpEmployee = function(employeeId) {
        $http.get($scope.apiUrl + "/employees/" + employeeId)
            .then(function(response) {
                //.data.data intentional
                var em = response.data.data;
                em.id = Number(em.id);
                em.salary = Number(em.salary);
                em.dep = Number(em.dep);
                em.phone1 = Number(em.phone1);
                em.phone2 = Number(em.phone2);
                em.bdate = new Date(em.bdate);
                $scope.updateemployee = em;
            });
    };
